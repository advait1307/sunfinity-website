...
export default function PresalesGears({ width = "100%", className = "" }) {
  const cwRef1 = useRef(null);
  const ccwRef = useRef(null);
  const cwRef2 = useRef(null);

  useEffect(() => {
    const noMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (noMotion) return;

    const animate = (el, duration, direction) => {
      if (!el) return;
      let start = null;
      let raf;
      const step = (ts) => {
  ...
  return (
    ...
    >
      ...
      {/* Blue connecting arrows (rendered behind gears) */}
      {ARROWS.map((d, i) => (
        <path
          key={i}
          d={d}
      ...
      {/* Static labels (sit above rotating gears, stay fixed) */}
      {GEARS.map((g) => {
        const lh = 21;
